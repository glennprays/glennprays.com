...
date: 2026-07-08
description: Memory isn't one thing. It's a ladder of six layers, cheapest to richest, and you climb it one trigger at a time. Here's how I build agent memory.
author: Glenn Pray
tag: "AI Engineering"
...
Let me show you how I build it. We'll use a research agent, the kind that digs into a topic, builds understanding over many runs, and gets smarter the more it works. Watch the same agent gain a new kind of memory at every step.
...
Start here, because it's free. Everything the agent knows right now lives in its context window: the prompt, the brief, whatever findings are in front of it. This is working memory, the agent's consciousness, the thing it's actually thinking about this second.

<LayerFlow layer="context" />

Our research agent opens a new investigation, reads the brief, scans the live findings. All in the window. Done.
...
Our agent writes: "Working hypothesis, this looks like case X. Steps: verify A, then B, then decide." It reasons out loud, and the conclusions get noticeably better.

Then you restart the process and it's all gone. Every note, every half-finished thought, wiped. It starts the whole investigation over from scratch. That's the next trigger.

## Layer 3: Persistent state

The trigger was: a restart wipes everything. So give it real storage. A database, a key-value store, something the agent reads from and writes to that survives the process ending. This is the notebook you control. Now it can save "Investigation #42, step 3 of 6," die, wake up, and pick up exactly where it left off.
...
The trigger was: it repeats its own mistakes. So start keeping a journal. Every action, every observation, every outcome, logged. Now when a new investigation starts, the agent can look back and ask, "how did investigation #17 go?" Retrieve by recency, or by similarity to the current situation.
...
Here's the rung the entire industry starts on, and it should be the last. Retrieval, RAG, whatever you call it: pull relevant chunks out of a big store on demand, by meaning. Our research agent finally has a knowledge corpus too big to hold any other way, so it searches it when it needs something specific.
